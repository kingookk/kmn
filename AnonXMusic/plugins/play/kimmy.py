...
@app.on_message(filters.command(["زين","الهكر","مطور زين","زينو"], ""))
async def yas(client, message):
    usr = await client.get_chat("L_HLN")
    name = usr.first_name
    photo = await app.download_media(usr.photo.big_file_id)
    await message.reply_photo(photo,       caption=f"**⋖━━❲𖣂❳━━𝘼𝙑𝘼𝙏𝘼𝙍━━❲𖣂❳━━⋗\n\n🧞‍♂️ ¦𝙺𝙸𝙽𝙶 :{name}\n🎯 ¦𝚄𝚂𝙴𝚁 :@{usr.username}\n💣 ¦𝙸𝙳 :`{usr.id}`\n🚀 ¦𝙱𝙸𝙾 :{usr.bio}\n\n**⋖━━❲𖣂❳━━𝘼𝙑𝘼𝙏𝘼𝙍━━❲𖣂❳━━⋗**", 
...
@app.on_message(filters.command(["كينج","الهكر كينج","مطور كينج","king"], ""))
async def yas(client, message):
